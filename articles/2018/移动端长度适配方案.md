...
假设我们的设计稿以iphone8的屏幕大小为准，即750*1334，其逻辑像素就是375px，那么根元素的字体大小可以这么计算：
...
实际开发中，建议在html文档一开始的地方就嵌入`js`代码来修改根元素html的字体大小，因为这样DOM在一开始渲染的时候拿的字体大小就是修改过的，如果在DOM渲染完成之后才修改字体大小，则修改字体后浏览器需要根据新的字体大小重新渲染一次。这样一来多渲染了一次，二来可能一开始会出现格式错乱，然后又马上恢复正常，体验不好。
...
@function px2rem($px, $base-font-size: 16px) {
  @return ($px / $base-font-size / 2) * 1rem; /* 设计稿为750，逻辑像素为375，两倍关系所有除以2 */
}
```
